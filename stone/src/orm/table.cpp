...
std::string Table::getFilename() const {
    return stone::ToLower(m_name + m_subfix);
}

bool Table::init(const tinyxml2::XMLElement& node) {
    if(!node.Attribute("name")) {
        STONE_LOG_ERROR(g_logger) << "table name is null";
        return false;
    }

    m_name = node.Attribute("name");
    if(!node.Attribute("namespace")) {
        STONE_LOG_ERROR(g_logger) << "table namespace is null";
        return false;
    }
    ...
    if(!cols) {
        STONE_LOG_ERROR(g_logger) << "table name=" << m_name << " columns is null";
        return false;
    }

    const tinyxml2::XMLElement* col = cols->FirstChildElement("column");
    if(!col) {
        STONE_LOG_ERROR(g_logger) << "table name=" << m_name << " column is null";
        return false;
    }
    ...
    do {
        ...
        if(!col_ptr->init(*col)) {
            STONE_LOG_ERROR(g_logger) << "table name=" << m_name << " init column error";
            return false;
        }
        if(col_names.insert(col_ptr->getName()).second == false) {
            STONE_LOG_ERROR(g_logger) << "table name=" << m_name << " column name="
    ...
    if(!idxs) {
        STONE_LOG_ERROR(g_logger) << "table name=" << m_name << " indexs is null";
        return false;
    }

    const tinyxml2::XMLElement* idx = idxs->FirstChildElement("index");
    if(!idx) {
        STONE_LOG_ERROR(g_logger) << "table name=" << m_name << " index is null";
        return false;
    }
    ...
    do {
        ...
        if(!idx_ptr->init(*idx)) {
            STONE_LOG_ERROR(g_logger) << "table name=" << m_name << " index init error";
            return false;
        }
        if(idx_names.insert(idx_ptr->getName()).second == false) {
            STONE_LOG_ERROR(g_logger) << "table name=" << m_name << " index name="
        ...
        if(idx_ptr->isPK()) {
            if(has_pk) {
                STONE_LOG_ERROR(g_logger) << "table name=" << m_name << " more than one pk"; 
                return false;
            }
            has_pk = true;
        }
        ...
        for(auto& x : cnames) {
            if(col_names.count(x) == 0) {
                STONE_LOG_ERROR(g_logger) << "table name=" << m_name << " idx="
...
void Table::gen(const std::string& path) {
    std::string p = path + "/" + stone::replace(m_namespace, ".", "/");
    stone::FSUtil::Mkdir(p);
    gen_inc(p);
    gen_src(p);
}

void Table::gen_inc(const std::string& path) {
    std::string filename = path + "/" + m_name + m_subfix + ".hpp";
    std::string class_name = m_name + m_subfix;
    std::string class_name_dao = m_name + m_subfix + "_dao";
    std::ofstream ofs(filename);
    ofs << "#ifndef " << GetAsDefineMacro(m_namespace + class_name + ".hpp") << std::endl;
    ofs << "#define " << GetAsDefineMacro(m_namespace + class_name + ".hpp") << std::endl;
    ...
    std::set<std::string> incs = {"src/db/db.hpp", "src/util.hpp"};
    for(auto& i : incs) {
        ofs << "#include \"" << i << "\"" << std::endl;
    }
    ofs << std::endl;
    ...
    std::vector<std::string> ns = stone::split(m_namespace, '.');
    ...
    ofs << "#endif //" << GetAsDefineMacro(m_namespace + class_name + ".hpp") << std::endl;
}
...
std::string Table::genToStringSrc(const std::string& class_name) {
    ...
    for(auto it = m_cols.begin();
        ...
        if((*it)->getDType() == Column::TYPE_UINT64
            ...
            ss << "stone::Time2Str(" << GetAsMemberName((*it)->getName()) << ")"
               << ";" << std::endl;
        } else {
    ...
    ss << "    return stone::JsonUtil::ToString(jvalue);" << std::endl;
    ss << "}" << std::endl;
    return ss.str();
}
...
void Table::gen_src(const std::string& path) {
    ...
    ofs << "#include \"" << class_name + ".hpp\"" << std::endl;
    ofs << "#include \"src/log.hpp\"" << std::endl;
    ofs << std::endl;
    ...
    ofs << std::endl;
    ofs << "static stone::Logger::ptr g_logger = STONE_LOG_NAME(\"orm\");" << std::endl;
...
#define CHECK_STMT(v) \
    ...
    ofs << "        STONE_LOG_ERROR(g_logger) << \"stmt=\" << sql" << std::endl << \
        "                 << \" errno=\"" \
