...
static stone::Logger::ptr g_logger_access = STONE_LOG_NAME("access");
...
std::string GetRemoteIP(stone::http::HttpRequest::ptr request
                        ,stone::http::HttpSession::ptr session) {
...
std::string Result::toJsonString() const {
    ...
    v["msg"] = msg;
    v["used"] = ((stone::GetCurrentUS() - used) / 1000.0);
    if(!jsondata.isNull()) {
        v["data"] = jsondata;
    } else {
        //if(!datas.empty()) {
    ...
    return stone::JsonUtil::ToString(v);
}
